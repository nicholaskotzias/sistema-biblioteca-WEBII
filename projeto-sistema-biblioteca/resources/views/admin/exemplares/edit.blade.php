<x-app-layout>
    <div class="min-h-screen flex flex-col justify-center items-center bg-gradient-to-br from-blue-50 via-white to-indigo-50
        ...
        <div class="max-w-xl w-full bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-8">
            ...
            <form action="{{ route('admin.exemplares.update', $exemplar->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="livro_id" name="livro_id" required
                        ...
                        @foreach ($livros as $livro)
                            <option value="{{ $livro->id }}" {{ old('livro_id', $exemplar->livro_id) == $livro->id ? 'selected' : '' }}>
                                {{ $livro->titulo }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="codigo_exemplar" name="codigo_exemplar" type="text" required
                        value="{{ old('codigo_exemplar', $exemplar->codigo_exemplar) }}"
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700
                               text-gray-900 dark:text-gray-100 focus:ring-blue-500 focus:border-blue-500" />
                </div>
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700
                               text-gray-900 dark:text-gray-100 focus:ring-blue-500 focus:border-blue-500">
                        @php
                            $statusOptions = ['DISPONIVEL' => 'Disponível', 'EMPRESTADO' => 'Emprestado', 'DANIFICADO' => 'Danificado', 'PERDIDO' => 'Perdido'];
                        @endphp
                        @foreach ($statusOptions as $value => $label)
                            <option value="{{ $value }}" {{ old('status', $exemplar->status) == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="localizacao" class="block text-gray-700 dark:text-gray-300 font-semibold mb-1">Localização</label>
                    <input id="localizacao" name="localizacao" type="text"
                        value="{{ old('localizacao', $exemplar->localizacao) }}"
